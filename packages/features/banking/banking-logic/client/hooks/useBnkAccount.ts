...
const transformResponse = <T>(data: any): T => {
  return data?.bnkAccount;
};

export const useBnkAccount = <T>(args: Args, fragments: Fragments): Result<T> => {
  const variables = { ...args };

  const { called, loading, data, error } = useQuery(
    assembleQuery(
      fragmentNames => `
         query BnkAccount($accountId: String) {
           bnkAccount(accountId: $accountId) {
             ${fragmentNames}
           }
         }
       `,
