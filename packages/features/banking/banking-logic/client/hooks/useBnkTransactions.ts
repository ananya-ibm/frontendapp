...
const transformResponse = <T>(data: any): T => {
  return data?.bnkTransactions ?? [];
};

export const useBnkTransactions = <T>(args: Args, fragments: Fragments): Result<T> => {
  const variables = { ...args };

  const { called, loading, data, error } = useQuery(
    assembleQuery(
      fragmentNames => `
         query BnkTransactions($accountId: String) {
           bnkTransactions(accountId: $accountId) {
             ${fragmentNames}
           }
         }
       `,
